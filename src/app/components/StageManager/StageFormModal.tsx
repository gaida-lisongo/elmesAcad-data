...
export default function StageFormModal({
  ...
  useEffect(() => {
    if (stage) {
      setDesignation(stage.designation || "");
      setDescription(stage.description || [""]);
      setPrix(stage.prix || 0);
      setDateDebut(
        stage.date_debut
          ? new Date(stage.date_debut).toISOString().split("T")[0]
          : "",
      );
      setDateFin(
        stage.date_fin
          ? new Date(stage.date_fin).toISOString().split("T")[0]
          : "",
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      let result;
      if (stage) {
        result = await updateStage(stage._id, {
          designation,
      ...
      if (result.success) {
        toast.success(
          stage ? "Stage modifié avec succès" : "Stage créé avec succès",
        );
        resetForm();
        onSuccess();
        onClose();
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Stage en entreprise 2024"
            />
          </div>
