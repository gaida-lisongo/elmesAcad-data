...
const Hero = ({ section }: { section: SectionType }) => {
  ...
  useEffect(() => {
    ...
    console.log('Hero mounted, hydrated:', hydrated, 'authenticated:', isAuthenticated())
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false)
      const action = (!section._id || section._id === '') ? 'créée' : 'mise à jour'
                  ...
                  {formData.promesses.map((promesse, index) => (
                    <div key={index} className='flex gap-2 items-center'>
                      ...
                      <input
                        type='text'
                        value={promesse}
                        onChange={(e) => handlePromesseChange(index, e.target.value)}
                        disabled={isLoading}
