    ...
    >
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-bodydark">
        <span className="flex items-center gap-1">
          <Icon icon="material-symbols:category-outline" width={14} />
          {document.category}
        </span>
        <span className="flex items-center gap-1">
          <Icon icon="material-symbols:payments" width={14} />
          {document.prix?.toLocaleString()} $
        </span>
        <span className="flex items-center gap-1">
          <Icon icon="material-symbols:event-outline" width={14} />
...
export default function FinanceDocumentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [releveResult, validationResult, metricsResult] = await Promise.all([
      getDocumentsByCategory("RELEVE", selectedPromotion.id, selectedAnnee._id),
    ...
    const docs = [
      ...(releveResult.success && releveResult.data ? releveResult.data : []),
      ...(validationResult.success && validationResult.data
        ? validationResult.data
        : []),
    ];

    if (releveResult.success || validationResult.success) {
      setDocuments(docs);
    } else {
  ...
  const selectedDocument = documents.find((d) => d._id === selectedId);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          ...
          <MetricCard
            ...
            sub="Transactions documentaires"
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-boxdark">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            ...
            <p className="text-sm text-bodydark">
              Cliquez sur un document pour voir ses commandes
            </p>
          </div>
          ...
          {documents.length === 0 ? (
            <div className="flex flex-col items-center py-10 text-bodydark">
              ...
              />
              <p className="text-sm">Aucun document pour cette promotion</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {documents.map((doc) => (
                <DocumentCard
                  ...
                  onClick={() =>
                    setSelectedId((prev) => (prev === doc._id ? null : doc._id))
                  }
                />
              ))}
            </div>
