...
export default class RelevePDF extends PDFManager {
  ...
  renderNotesSection() {
    ...
    notes.forEach((noteUnite) => {
      ...
      body.push([
        ...
        { text: String(moyenneUnite), alignment: "center" },
  ...
  async render(
    ...
    const docDefinition = this.renderDocument(
      ...
      {
        reference: metadata?.reference || `REL-${Date.now()}`,
    ...
    const resolvedFileName =
      fileName ||
      `Releve_${this.studentNote.nomComplet.replace(/\s+/g, "_")}_${Date.now()}.pdf`;
